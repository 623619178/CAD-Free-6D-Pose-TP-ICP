...
def _load_pem_model_once(config_path="config/base.yaml", gpus="0"):
    """Load PEM model weights exactly once. Returns (model, cfg)."""
    cfg = gorilla.Config.fromfile(config_path)
    cfg.gpus = gpus
    cfg.model_name = "pose_estimation_model"
    cfg.exp_name = "pose_estimation_model_base_id0"
    cfg.log_dir = "log/pose_estimation_model_base_id0"
    ...
    print("PEM Persistent: Creating model (this is the only time)...")
    ...
    print("PEM Persistent: Model loaded!")

    return model, cfg


def _run_pem_frame(model, cfg, output_dir, cad_path, rgb_path, depth_path, cam_path, seg_path, det_score_thresh=0.32):
    """Run PEM inference for a single frame using a pre-loaded model.
    This replicates the original __main__ logic (lines 272-318), but without model loading."""
    random.seed(cfg.rd_seed)
    torch.manual_seed(cfg.rd_seed)

    # --- Extract templates for this frame ---
    tem_path = os.path.join(output_dir, 'templates')
    ...
    input_data, img, whole_pts, model_points, detections = get_test_data(
        ...
        det_score_thresh, cfg.test_dataset
    )
...
def run_persistent_server():
    """Persistent server: load PEM model ONCE, then process frames via stdin/stdout JSON protocol.

    Protocol:
      - Server writes '###PEM_READY###' to stdout when model is loaded.
      - Client writes one JSON object per line to stdin:
        {output_dir, cad_path, rgb_path, depth_path, cam_path, seg_path, det_score_thresh}
      - Server writes one JSON response per line to stdout:
        {status: 'ok'} or {status: 'error', message: '...'}
      - Client writes '###EXIT###' to shut down the server.

    All regular print goes to stderr so stdout stays clean for protocol.
    """
    ...
    print("PEM Server: Loading model...")
    model, cfg = _load_pem_model_once()
    print("PEM Server: Model loaded! Entering serve loop.")

    _protocol_out.write("###PEM_READY###\n")
    _protocol_out.flush()
    ...
    for line in sys.stdin:
        ...
        try:
            ...
            _run_pem_frame(
                ...
                cmd['seg_path'], cmd.get('det_score_thresh', 0.32)
            )
            _protocol_out.write(_json.dumps({"status": "ok"}) + "\n")
            _protocol_out.flush()
        except Exception as e:
            print(f"PEM Server frame error: {e}", flush=True)
            import traceback
            traceback.print_exc()
            _protocol_out.write(_json.dumps({"status": "error", "message": str(e)}) + "\n")
            _protocol_out.flush()

    print("PEM Server: Shutting down.")


if __name__ == "__main__":
    # Check for persistent mode BEFORE init() parses args
    if "--persistent" in sys.argv:
        ...
        all_tem, all_tem_pts, all_tem_choose = get_templates(tem_path, cfg.test_dataset)
        print(f"=> templates extracted: {len(all_tem)}")
        ...
        print(f"=> input data loaded. detections: {len(detections)}")
        ninstance = input_data['pts'].size(0)
        print(f"=> ninstance: {ninstance}")
